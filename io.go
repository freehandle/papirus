...
func (j *JournalFileStore) AsByteStore() ByteStore {
	fileName := j.file.Name()
	j.file.Close()
	bytestore := OpenFileStore(fileName)
	return bytestore
}
...
func Clone(src, dst ByteStore) {
	offset := int64(0)
	size := src.Size()
	done := false
	for {
		step := int64(maxCloningBlockSize)
		if offset+maxCloningBlockSize >= size {
			step = size - offset - 1
			done = true
		}
		bytes := src.ReadAt(offset, step)
		dst.WriteAt(offset, bytes)
		offset += step
		if done {
			return
		}
...
func (f *FileStore) ReadAt(offset int64, nbytes int64) []byte {
	if offset+nbytes > f.size || offset < 0 || nbytes < 1 { // >= ou >?
		panic(fmt.Sprintf("invalid offset %v, %v, %v", offset, nbytes, f.size))
	}
